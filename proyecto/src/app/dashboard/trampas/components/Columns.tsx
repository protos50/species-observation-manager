...
type HandleTrapAction = (
  trapId: number,
  trapName: string,
  action: "delete" | "restore"
) => Promise<void>;

type HandleTrapEdit = (trap: Trap) => void;

export function getColumns(
  handleTrapAction: HandleTrapAction,
  onEdit?: HandleTrapEdit
): ColumnDef<Trap>[] {
  return [
    {
      accessorKey: "trap_name",
      header: "Nombre de trampa",
      meta: {
        align: "center",
      },
    },
    {
      ...
      cell: ({ row }) => {
        const trap = row.original;

        const handleDelete = () => {
          void handleTrapAction(trap.id_trap, trap.trap_name, "delete");
        };
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const handleRestore = async () => {
          try {
            await handleTrapAction(trap.id_trap, trap.trap_name, "restore");
            toast.success(
              `La trampa "${trap.trap_name}" fue restaurada exitosamente`
            );
          } catch {
            toast.error("Error al restaurar la trampa");
          }
        };
