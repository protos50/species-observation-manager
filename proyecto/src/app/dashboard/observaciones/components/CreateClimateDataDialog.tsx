...
export function CreateClimateDataDialog({
  ...
  // Rellenar formulario con datos iniciales en modo edición
  useEffect(() => {
    if (open && initialData) {
      form.reset({
        t_min: initialData.t_min?.toString() || "",
        t_max: initialData.t_max?.toString() || "",
        t_med: initialData.t_med?.toString() || "",
        hr_min: initialData.hr_min?.toString() || "",
        hr_max: initialData.hr_max?.toString() || "",
        hr_med: initialData.hr_med?.toString() || "",
        pp_14_days_before: initialData.pp_14_days_before?.toString() || "",
        pp_30_days_before: initialData.pp_30_days_before?.toString() || "",
      });
    } else if (open) {
  ...
  const onSubmit = async (values: ClimateFormValues) => {
    ...
    try {
      const climateDate = date + "T12:00:00.000Z";
      ...
      const createData = {
        id_locality: parseInt(localityId),
      ...
      if (!response.ok) {
        throw new Error("Error al crear datos climáticos");
      }
      ...
      toast.error("Error al crear datos climáticos");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] p-0">
        <DialogHeader className="px-6 pt-6">
          <DialogTitle className="text-2xl">
            {initialData ? "Editar Datos Climáticos" : "Crear Datos Climáticos"}
          </DialogTitle>
          <DialogDescription>
            {initialData ? "Modifique" : "Ingrese"} los datos climáticos para {localityName || "la localidad"} el{" "}
            {date}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col">
            ...
            <DialogFooter className="px-6 py-4 border-t">
              ...
              <Button type="submit" disabled={isLoading}>
                {isLoading ? "Guardando..." : (initialData ? "Actualizar Datos Climáticos" : "Crear Datos Climáticos")}
              </Button>
            </DialogFooter>
          </form>
        </Form>
