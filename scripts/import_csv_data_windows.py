                ...
                self.cursor.execute(
                    '''INSERT INTO public."Observation" 
                       (id_taxon, id_collection, id_geolocation, id_environment, id_caste, id_climate_data,
                        abundance, id_identifier, identification_date, biology_notes, 
                        general_observations, conservation_status)
                       VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id_observation''',
                    (id_taxon, id_collection, id_geolocation, id_environment, id_caste, id_climate_data,
                     abundance, id_identifier, identification_date, biology_notes,
                     general_observations, conservation_status)
                )
...
def main():
    """Función principal"""
    csv_path = Path(__file__).with_name("WZNHbvR.xlsx")
    
    if not csv_path.exists():
        print(f"❌ Archivo no encontrado: {csv_path}")
        print("💡 Verifica que el archivo WZNHbvR.xlsx exista en la ruta especificada")
        return 1
